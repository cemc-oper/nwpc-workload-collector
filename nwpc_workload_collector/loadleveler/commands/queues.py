...
from nwpc_workload_collector.loadleveler.common.llclass import get_llclass_query_model


def collect_queues(config, owner, repo,
    ...
    query_model = get_llclass_query_model(client, config)
    ...
    if output_style == 'show':
        ...
        if verbose:
            print("Posting loadleveler queues for {owner}/{repo}...".format(owner=owner, repo=repo))
        ...
        if verbose:
            print("Posting loadleveler queues for {owner}/{repo}...done".format(owner=owner, repo=repo))
        return None


@click.command('queues', short_help='show queues information in loadleveler')
...
def command(config_file, owner, repo,
            host, port, user, password,
            output_style, post_url, content_encoding, verbose):
    config = get_config(config_file)
    return collect_queues(config, owner, repo,
                          host, port, user, password,
                          output_style, post_url, content_encoding, verbose)
