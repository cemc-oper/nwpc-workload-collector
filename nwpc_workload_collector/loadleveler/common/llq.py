...
def run_llq_command(client: SSHClient, command="/usr/bin/llq -l", params="") -> (str, str):
    """
...
def get_llq_query_model(client: SSHClient, config, params=""):
    """
    ...
    """
    std_out_string, std_error_out_string = run_llq_command(client, params=params)
    output_lines = std_out_string.split("\n")
    category_list = build_category_list(config['llq']['category_list'])
