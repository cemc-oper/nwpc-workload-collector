...
from nwpc_workload_collector.slurm.server.proto import (
    slurm_collector_pb2_grpc, slurm_collector_pb2)
...
def jobs(rpc_target, owner, repo,
    ...
    jobs_request = slurm_collector_pb2.JobsRequest(
        owner=owner,
        repo=repo,
        host=host,
        port=port,
    ...
    with grpc.insecure_channel(rpc_target) as channel:
        stub = slurm_collector_pb2_grpc.SlurmCollectorStub(channel)
...
def queues(rpc_target, owner, repo,
           host, port, user, password,
           output_style, post_url, content_encoding, verbose):
    queues_request = slurm_collector_pb2.QueuesRequest(
        owner=owner,
        repo=repo,
        host=host,
        port=port,
