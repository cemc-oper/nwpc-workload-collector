...
from nwpc_workload_collector.slurm.commands.jobs import collect_jobs
from nwpc_workload_collector.slurm.commands.queues import collect_queues
from nwpc_workload_collector.slurm.server.proto import (
    slurm_collector_pb2, slurm_collector_pb2_grpc)
...
class SlurmCollectorService(slurm_collector_pb2_grpc.SlurmCollectorServicer):
    ...
    def CollectJobs(self, request, context):
        ...
        response = slurm_collector_pb2.Response(
            status="ok"
        )
...
def serve(rpc_target, workers_num, config_file_path):
    ...
    slurm_collector_pb2_grpc.add_SlurmCollectorServicer_to_server(
        SlurmCollectorService(config), server)
    ...
    logger.info('starting slurm collector server...')
    server.start()
    try:
        while True:
